...
use deltalake_core::storage::{factories, ObjectStoreFactory, ObjectStoreRef, StorageOptions, url_prefix_handler};
...
impl ObjectStoreFactory for HdfsFactory {
    fn parse_url_opts(
        ...
        let hdfs = HadoopFileSystem::new(url.as_str()).ok_or_else(|| {
            DeltaTableError::Generic(format!("Failed to create HadoopFileSystem from url: {}", url))
        })?;
